 ...
 * @return string
 */
function option_tree_background_pattern( $value, $settings, $int ) { ?>
			...
			while (false !== ($file = readdir($handle))) {
				if($file != '..' && $file != '.') {
					...
					echo '<div class="single_pattern">
							...
							<label class="label-for-check" for="'.$value->item_id.'_'.$count.'"><div class="pattern" style="background: url('.get_template_directory_uri().'/../nevada/images/pattern/'.$file.');"></div></label></div>';
					$count++;
				}
			}
