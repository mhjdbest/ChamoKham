...
/**
 * Background Option with Texture
 *
 * @since Lambda 2.0
 *
 * @param array $value
 ...
 * @return string
 */
function option_tree_background_texture( $value, $settings, $int ) { ?>
			...
			while (false !== ($file = readdir($handle))) {
				if($file != '..' && $file != '.') {
					...
					echo '<div class="single_pattern">
							...
							<label class="label-for-check" for="'.$value->item_id.'_'.$count.'"><div class="pattern" style="background: url('.get_template_directory_uri().'/../nevada/images/bg-textured/prev/'.$file.');"></div></label></div>';
					$count++;
				}
			}
