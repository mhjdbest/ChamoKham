...
if($login_required == 'yes' && !$is_logged_in) {
	...
	$error = 1;
	$msg = __('You need to be logged in to vote!', UT_THEME_NAME);
...
if($can_vote) {
	...
	$query = ($has_already_voted) ?
		"UPDATE {$wpdb->prefix}lambda_like_post SET likeit = likeit + 1, date_time = '" . date('Y-m-d H:i:s') . "' WHERE post_id = '" . $post_id . "' AND ip = '$ip'" :
		"INSERT INTO {$wpdb->prefix}lambda_like_post SET post_id = '" . $post_id . "', likeit = '1', date_time = '" . date('Y-m-d H:i:s') . "', ip = '$ip'";
	
	$success = $wpdb->query($query);
	if($success) {
		
		$error = 0;
		$msg = __('Thanks for voting!', UT_THEME_NAME);
	
	} else {
		
		$error = 1;
		$msg = __('Sorry, a technical error has occured. Please try again later', UT_THEME_NAME);
	}
}
