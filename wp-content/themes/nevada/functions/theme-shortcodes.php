...
// Latest Posts
function lambda_latest($atts, $content = null) {
	extract(shortcode_atts(array(
	"num" => '5',
	"thumbs" => 'false',
	"excerpt" => 'false',
	...
	$myposts = new WP_Query('cat='.$cat.'&posts_per_page='.$num.'&orderby=polambda_date&order=DESC');

	$result='<ul class="captionlist">';

	while($myposts->have_posts()) : $myposts->the_post();
		$result.='<li class="clearfix">';
			if (has_polambda_thumbnail() && $thumbs == 'true') {
				$result.= '<img alt="'.get_the_title().'" class="alignleft" src="'.aq_resize( get_image_path(), $width, $height, true ).'"/>';
			}
...
add_shortcode("latest", "lambda_latest");
// Example Use: [latest excerpt="true" thumbs="true" width="50" height="50" num="5" cat="8,10,11"]


// Related Posts - [related_posts]
add_shortcode('related_posts', 'lambda_related_posts');
function lambda_related_posts( $atts ) {
	extract(shortcode_atts(array(
	    'limit' => '5',
	), $atts));
	...
	if ($post->ID) {
		$retval = '<div class="lambda_relatedposts">';
		$retval .= '<h4>Related Posts</h4>';
		$retval .= '<ul>';
 		// Get tags
		$tags = wp_get_polambda_tags($post->ID);
		$tagsarray = array();
		foreach ($tags as $tag) {
		...
		$q = "SELECT p.*, count(tr.object_id) as count
			FROM $wpdb->term_taxonomy AS tt, $wpdb->term_relationships AS tr, $wpdb->posts AS p WHERE tt.taxonomy ='polambda_tag' AND tt.term_taxonomy_id = tr.term_taxonomy_id AND tr.object_id  = p.ID AND tt.term_id IN ($tagslist) AND p.ID != $post->ID
				AND p.polambda_status = 'publish'
				AND p.polambda_date_gmt < NOW()
 			GROUP BY tr.object_id
			ORDER BY count DESC, p.polambda_date_gmt DESC
			LIMIT $limit;";

		$related = $wpdb->get_results($q);
 		if ( $related ) {
			foreach($related as $r) {
				$retval .= '<li><a title="'.wptexturize($r->polambda_title).'" href="'.get_permalink($r->ID).'">'.wptexturize($r->polambda_title).'</a></li>';
			}
		} else {
...
class lambda_table_shortcode {
	...
	function handle_table_shortcode($atts) {
		...
		$table_name = $wpdb->prefix . "lambda_tables"; 
