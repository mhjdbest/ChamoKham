...
if ( !function_exists( 'lambda_main_slider' ) ) {
	...
	function lambda_main_slider($slides) {
		...
		if( $sliderinfo[0] == 'revslider' ) {
			...
			echo do_shortcode('[rev_slider '.$sliderinfo[1].']');
...
if ( !function_exists( 'GetLambdaLikeCount' ) ) {
	function GetLambdaLikeCount($post_id) {
		 ...
		 $lambda_like_count = $wpdb->get_var("SELECT SUM(likeit) FROM {$wpdb->prefix}lambda_like_post WHERE post_id = '$post_id' AND likeit >= 0");
...
if ( !function_exists( 'HasLambdaAlreadyVoted' ) ) {
	function HasLambdaAlreadyVoted($post_id, $ip = null) {
		 ...
		 $lambda_has_voted = $wpdb->get_var("SELECT COUNT(id) AS has_voted FROM {$wpdb->prefix}lambda_like_post WHERE post_id = '$post_id' AND ip = '$ip'");
...
if ( !function_exists( 'LambdaMostLikeQuery' ) ) {
	function LambdaMostLikeQuery($limit, $where) {
		...
		$query = "SELECT post_id, SUM(likeit) AS like_count, post_title FROM `{$wpdb->prefix}lambda_like_post` L, {$wpdb->prefix}posts P ";
