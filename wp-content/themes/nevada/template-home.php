...
if( !function_exists( 'home_blog_columns' ) ) {
    function home_blog_columns($count) {
		...
		<?php if(isset($metadata['blog_headline'])) :?>
			...
			<h3 class="home-title"><span><?php echo $metadata['blog_headline'] ?></span></h3>
		...
		<section class="clearfix<?php echo ($count == 1) ? ' home-border' : ''; ?>">
			...
			<div class="lambda-service-excerpt one_half">
				...
				<?php if(isset($metadata['service_headline'])) :?>
					...
					<h3 class="home-title"><span><?php echo $metadata['service_headline'] ?></span></h3>
				...
				<div class="clearfix">
					...
					<article class="list"><h3 class="trigger"><a href="#"><?php echo (isset($tab['tab_name'])) ? $tab['tab_name'] : ''; ?></a></h3>
...
if( !function_exists( 'home_clients' ) ) {
    function home_clients($count) {
		...
		//receive standard meta
		$metadata = $lambda_meta_data->the_meta();
		$clients = $metadata[UT_THEME_INITIAL.'home_clients'];
		...
		<section class="client-wrap clearfix">	
			...
			<?php if(isset($metadata['client_headline'])) :?>
				...
				<h3 class="home-title"><span><?php echo $metadata['client_headline'] ?></span></h3>
...
# Loop Home Items 
#-----------------------------------------------------------------
if(is_array($metadata['home_item'])) {
	...
	foreach($metadata['home_item'] as $home_item) {		
		...
		switch($home_item) {
			...
			if(isset($metadata['activate_cta']) && $metadata['activate_cta'] == 'on') {
				...
				echo do_shortcode('[cta ctaclass="margin-40" headline="'.$metadata['cta_headline'].'" buttonlink="'.$buttonlink.'" buttontext="'.$buttontext.'"] '.$metadata['cta_content'].' [/cta]');
