...
<?php
			...
			if(is_array($taxonomys)) {
				...
				foreach ($taxonomys as $key => $value ) {
					...
					if($key == 0 && isset($taxonomys[$key]->name) && in_array($taxonomys[$key]->name, $used_taxonomys)) {
						echo '<li><a href="#" data-filter="*" class="selected">'.$taxonomys[$key]->name.'</a></li>';
					} 
					if(in_array($taxonomys[$key]->name, $used_taxonomys) && $key != 0 ) {							
						 ...
						 ?>
								...
								<a href="<?php echo get_permalink(); ?>">
									...
									<div class="hover-overlay">
										<?php if($layout['portfolio_item_title'] != 'on') { ?>
