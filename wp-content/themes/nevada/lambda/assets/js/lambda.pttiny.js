...
/*-----------------------------------------------------------------------------------*/
	...
	sortable: function(){
		...
		$('#wpa_loop-nevada_verticaltabs').sortable({
			//cancel: ':input,button,.customEditor', // exclude TinyMCE area from the sort handle
		...
		$('#wpa_loop-nevada_home_verticaltabs').sortable({
			//cancel: ':input,button,.customEditor', // exclude TinyMCE area from the sort handle
			handle: '.testimonial_item_name',
			placeholder: "verticaltabs_highlight",
			opacity: 0.5,
		...
		$('#wpa_loop-nevada_home_items').sortable({
		...
		$('#wpa_loop-nevada_home_testimonials').sortable({
			opacity: 0.5,
			tolerance: 'pointer',
			start: function(event, ui) { // turn TinyMCE off while sorting (if not, it won't work when resorted)
                ui.placeholder.height(ui.item.height());
		...
		$('#wpa_loop-nevada_testimonial_items').sortable({
			opacity: 0.5,
			tolerance: 'pointer',
			start: function(event, ui) { // turn TinyMCE off while sorting (if not, it won't work when resorted)
                ui.placeholder.height(ui.item.height());
		...
		$('#wpa_loop-nevada_team_member').sortable({
			opacity: 0.5,
			tolerance: 'pointer',
			start: function(event, ui) { // turn TinyMCE off while sorting (if not, it won't work when resorted)
                ui.placeholder.height(ui.item.height());
		...
		$('#wpa_loop-nevada_contact_form_fields').sortable({
			opacity: 0.5,
			tolerance: 'pointer',
			start: function(event, ui) { // turn TinyMCE off while sorting (if not, it won't work when resorted)
                ui.placeholder.height(ui.item.height());
		...
		$('#wpa_loop-nevada_faq_items').sortable({
			opacity: 0.5,
			tolerance: 'pointer',
			start: function(event, ui) { // turn TinyMCE off while sorting (if not, it won't work when resorted)
                ui.placeholder.height(ui.item.height());
		...
		$('#wpa_loop-nevada_project_atts').sortable({
			opacity: 0.5,
			tolerance: 'pointer',
			start: function(event, ui) { // turn TinyMCE off while sorting (if not, it won't work when resorted)
                ui.placeholder.height(ui.item.height());
		...
		$('#wpa_loop-nevada_portfolio_images').sortable({
			opacity: 0.5,
			tolerance: 'pointer',
			start: function(event, ui) { // turn TinyMCE off while sorting (if not, it won't work when resorted)
                ui.placeholder.height(ui.item.height());
