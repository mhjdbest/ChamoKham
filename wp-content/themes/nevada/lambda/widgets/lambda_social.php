...
class WP_Widget_Social extends WP_Widget {
    ...
    function __construct() {
		$widget_ops = array('classname' => 'lambda_widget_social clearfix', 'description' => __( 'Displays Social messages by User!', UT_THEME_NAME) );
		parent::__construct('lw_social', __('Lambda - Social', UT_THEME_NAME), $widget_ops);
    ...
    function form($instance) {
		...
		if ($handle = opendir($absolute_path_to_theme.'/images/icons/social/black/')) {
			...
			while (false !== ($file = readdir($handle))) {
				if($file != '..' && $file != '.') {
					...
					//Create Key Name
					$key = ereg_replace('.png', '', $file);
					$key = ereg_replace('_alt', '', $key);
					$key = ucfirst($key);					
		...
		<script type="text/javascript">
			jQuery(document).ready(function($){
				...
				var item = '<div class="widget-item lambda-widget-box"><p><label><?php _e('Icon', UT_THEME_NAME); ?>:</label>';
				...
				item  += '<p><label><?php _e('Link', UT_THEME_NAME); ?>:</label><input type="text" class="widefat link" /></p>';
				item += '<a href="#" class="del-item button clear"><?php _e('remove', UT_THEME_NAME); ?></a></div>';
		...
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', UT_THEME_NAME); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></p>


		<p><label for="<?php echo $this->get_field_id('color'); ?>"><?php _e('Color:', UT_THEME_NAME); ?></label>
		<select name="<?php echo $this->get_field_name('color'); ?>">
			...
			<option value="black" <?php echo $checked_black; ?>><?php _e('Black Icons', UT_THEME_NAME); ?></option>	
			<option value="white" <?php echo $checked_white; ?>><?php _e('White Icons', UT_THEME_NAME); ?></option>
		...
		if(is_array($items)) {
			...
			<div class="widget-item lambda-widget-box">
				...
				<p><label><?php _e('Icon', UT_THEME_NAME); ?>:</label>
				...
				<p><label><?php _e('Link', UT_THEME_NAME); ?>:</label><input type="text" class="link" value="<?php echo esc_attr($fields['link']); ?>" /></p>
				<div class="clear"></div>
				<a href="#" class="del-item button"><?php _e('remove', UT_THEME_NAME); ?></a>
			</div>
		...
		<div class="widget-item lambda-widget-box">
			...
			<p><label><?php _e('Link', UT_THEME_NAME); ?>:</label><input type="text" class="link" /></p>
	...
	if(is_array($items)) {
		...
		foreach( $items as $num => $fields ){
			...
			//Create "Alt"
			$alt = ereg_replace('.png', '', $fields['icon']);
			$alt = ereg_replace('_alt', '', $alt);
