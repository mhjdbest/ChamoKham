...
global $lambda_meta_data, $emailSent, $captcha_error, $columns;
...
<section id="post-<?php the_ID(); ?>" <?php post_class('single'); ?>>
	...
	<?php 
		...
		if(isset($contactelements['show_map'])) {
			...
			echo '<h3 class="home-title"><span>'.$contactelements['map_title'].'</span></h3>';
			
			//echo '<figure class="img-holder"><img src="http://maps.google.com/maps/api/staticmap?center='.urlencode($lambda_meta_data->get_the_value('google')).'&amp;zoom=14&amp;markers='.urlencode($lambda_meta_data->get_the_value('google')).'&amp;size=640x300&amp;sensor=false" /></figure>'; 
    ...
    <?php
		...
		if(isset($contactelements['contact_title'])) {
			echo '<h3 class="home-title"><span>'.$contactelements['contact_title'].'</span></h3>';
		}
	...
	#----------------------------------------------------------------- 
	if ($emailSent) {
		echo '<div class="alert green" id="send_success">'.__( 'Message has been sent!', UT_THEME_NAME ).'</div>';
	}
	if ($captcha_error) {
		echo '<div class="alert yellow" id="send_error">'.__( 'Message has not been sent! Reason: Incorrect Caption!', UT_THEME_NAME ).'</div>';
	}	
    ...
    <form action="<?php the_permalink(); ?>#send_success" id="contactForm" method="post" class="validateform">
    		 <?php 		
						...
						if(isset($contactelements[UT_THEME_INITIAL.'contact_form_fields']) && is_array($contactelements[UT_THEME_INITIAL.'contact_form_fields'])) {
							...
							echo '<p><input type="submit" value="'.$contactelements['submit_text'].'" ></p>';
						}	
			?>
